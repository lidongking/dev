...
class BramusRouter
{
    ...
    public function before($methods, $pattern, $fn)
    {
        ...
        foreach (explode('|', $methods) as $method) {
            $this->currentRoutes[$method][] = array(
                'pattern' => $pattern,
                'fn' => $fn,
            );
        }
    ...
    public function after($methods, $pattern, $fn)
    {
        $pattern = $this->baseRoute.'/'.trim($pattern, '/');
        $pattern = $this->baseRoute ? rtrim($pattern, '/') : $pattern;
    ...
    public function run($callback = null)
    {
        ...
        // Handle all routes
        $numHandled = 0;
        if (isset($this->currentRoutes[$this->requestedMethod])) {
            $numHandled = $this->handle($this->currentRoutes[$this->requestedMethod], true);
        }
        ...
        // Handle all after after
        if (isset($this->afterRoutes[$this->requestedMethod])) {
            $this->handle($this->afterRoutes[$this->requestedMethod], true);
        }
